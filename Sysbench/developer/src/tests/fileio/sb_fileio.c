...
void file_report_cumulative(sb_stat_t *stat)
{
  ...
  if(test_mode == MODE_RND_READ) sb_globals.fileio_read_iops = stat->reads / seconds;
  else if(test_mode == MODE_RND_WRITE) sb_globals.fileio_write_iops = stat->writes / seconds;
  else if(test_mode == MODE_READ) sb_globals.fileio_read_bandwith = stat->bytes_read / megabyte / seconds;
  else if(test_mode == MODE_WRITE) sb_globals.fileio_write_bandwith = stat->bytes_written / megabyte / seconds;
  // log_text(LOG_NOTICE, "");
