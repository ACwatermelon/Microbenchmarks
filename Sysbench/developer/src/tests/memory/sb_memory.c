...
void memory_report_cumulative(sb_stat_t *stat)
{
  ...
  if (memory_oper != SB_MEM_OP_NONE)
  {
    ...
    if(memory_oper == SB_MEM_OP_READ) sb_globals.memory_read_bandwith = mb / stat->time_interval;
    else if(memory_oper == SB_MEM_OP_WRITE) sb_globals.memory_write_bandwith = mb / stat->time_interval;
